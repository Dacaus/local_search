...
* 枚举局部 schedule matrix
* 实现 legality check
* 构建 locality proxy score
* 绘制 schedule landscape heatmap

## 尝试验证

1. 邻近合法调度是否具有相近性能？

2. schedule space 是否呈现连续结构，而非完全随机黑盒？

3. 是否存在低维流形结构，可供学习模型逼近？


## 下一步
* 接入真实 benchmark： matmul jacobi stencil
* 使用 clang + Polly 获取真实调度
* 测量真实运行时间
* 将 proxy score 替换为真实 performance